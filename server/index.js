  ...
  try {
    for (const p of pairsData) {
      ...
      `, [
        ...
        JSON.stringify(p.base_token), JSON.stringify(p.quote_token),
        p.base_symbol, p.quote_symbol, p.dex, p.pool_address,
        p.base_token_decimals, p.quote_token_decimals,
        JSON.stringify(p.base_token_info), JSON.stringify(p.quote_token_info),
...
const fetchPairsFromDB = async (network) => {
  try {
    let query = `SELECT * FROM pairs ORDER BY indexed_at DESC LIMIT 200`;
    const params = [];
    if (network) {
      query = `SELECT * FROM pairs WHERE network = $1 ORDER BY indexed_at DESC LIMIT 200`;
      params.push(network);
    }
