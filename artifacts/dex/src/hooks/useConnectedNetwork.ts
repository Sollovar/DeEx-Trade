...
const STORAGE_KEY = "nexus-v1-network";
...
const getSolanaProvider = ():
  | { isPhantom?: boolean; isSolflare?: boolean; publicKey?: { toString: () => string }; isConnected?: boolean }
  | null => {
...
const getEthereumProvider = (): {
  ...
  if (win.okxwallet) return win.okxwallet as { request: (args: { method: string }) => Promise<string> };
  if (win.leapwallet) return win.leapwallet as { request: (args: { method: string }) => Promise<string> };
  if (win.cosmostation) return win.cosmostation as { request: (args: { method: string }) => Promise<string> };
  if (win.keplr) return win.keplr as { request: (args: { method: string }) => Promise<string> };
  if (win.ethereum) return win.ethereum as { request: (args: { method: string }) => Promise<string> };
  return null;
};
...
export function useConnectedNetwork(): Network {
  ...
  const detectNetwork = useCallback(async () => {
    ...
    try {
      if (dynamicNetwork) {
        const normalized = normalizeNetworkValue(dynamicNetwork as string);
        if (normalized) {
  ...
  useEffect(() => {
    ...
    initNetwork();
  }, [primaryWallet, dynamicNetwork, setNetworkState]);
  ...
  useEffect(() => {
    const ethereum = getEthereumProvider();
    if (!ethereum) return;
    const handleChainChange = () => detectNetwork();
    if (ethereum.on) ethereum.on("chainChanged", handleChainChange);
    return () => ethereum.removeListener?.("chainChanged", handleChainChange);
  }, [detectNetwork]);

  useEffect(() => {
    const handleManualChange = () => setNetwork(getStoredNetwork());
