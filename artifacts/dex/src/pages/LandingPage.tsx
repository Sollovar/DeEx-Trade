...
export function LandingPage() {
  ...
  const NAV_LINKS = [
    { key: "nav.trade",     label: t('nav.trade') },
    { key: "nav.portfolio", label: t('nav.portfolio') },
    { key: "nav.chain",     label: "Chain" },
    { key: "nav.rewards",   label: "Rewards" },
    { key: "nav.docs",      label: "Docs" },
  ];

  const WHY_CARDS = [
    { icon: Globe, titleKey: 'landing.why.universal.title', descKey: 'landing.why.universal.desc', delay: 0 },
    { icon: Lock,  titleKey: 'landing.why.privacy.title',   descKey: 'landing.why.privacy.desc',   delay: 0.08 },
    { icon: Zap,   titleKey: 'landing.why.capital.title',   descKey: 'landing.why.capital.desc',   delay: 0.16 },
    { icon: Layers,titleKey: 'landing.why.composability.title', descKey: 'landing.why.composability.desc', delay: 0.24 },
  ];

  const CHAIN_CARDS = [
    { icon: Zap,    titleKey: 'landing.chain.finality.title',  descKey: 'landing.chain.finality.desc' },
    { icon: Shield, titleKey: 'landing.chain.privacy.title',   descKey: 'landing.chain.privacy.desc' },
    { icon: Layers, titleKey: 'landing.chain.multichain.title',descKey: 'landing.chain.multichain.desc' },
  ];

  const FEATURE_ROWS = [
    { badgeKey: 'landing.row1.badge', titleKey: 'landing.row1.title', descKey: 'landing.row1.desc', accent: "24/7 · Permissionless", reverse: false, bg: "bg-black" },
    { badgeKey: 'landing.row2.badge', titleKey: 'landing.row2.title', descKey: 'landing.row2.desc', accent: "ZK-Verified · Stealth Addresses", reverse: true, bg: "bg-[#050505]" },
    { badgeKey: 'landing.row3.badge', titleKey: 'landing.row3.title', descKey: 'landing.row3.desc', accent: "Yield Bearing Stablecoin · Dual Yield", reverse: false, bg: "bg-black" },
  ] as const;

  const FOOTER_COLS = [
    { labelKey: 'landing.footer.product', links: ["Trade", "Portfolio", "Mobile App", "Roadmap"] },
    { labelKey: 'landing.footer.learn',   links: ["Docs", "Whitepaper", "Blog", "Tutorials"] },
    { labelKey: 'landing.footer.chain',   links: ["Explorer", "Bridge", "Validators", "Grant"] },
    { labelKey: 'landing.footer.company', links: ["About", "Careers", "Press", "Contact"] },
  ];
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden font-sans">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-white/50">
            {NAV_LINKS.map(l => (
              <a key={l.key} href="#" className="hover:text-white transition-colors duration-200">{l.label}</a>
            ))}
          </div>
          ...
          {menuOpen && (
            ...
            >
              <div className="px-6 py-4 flex flex-col gap-4 text-sm text-white/60">
                {NAV_LINKS.map(l => <a key={l.key} href="#" className="hover:text-white transition-colors">{l.label}</a>)}
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-[88px] pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-[#f5c518]">{t('landing.heroHighlight')}</span>
          </motion.h1>
          ...
          >
            <StatCard label={t('landing.stat.assets')} value={380} />
            <StatCard label={t('landing.stat.openInterest')} value={5438} prefix="$" suffix="M" />
            <StatCard label={t('landing.stat.users')} value={12400} />
            <StatCard label={t('landing.stat.volume')} value={4441} prefix="$" suffix="M" />
          </motion.div>
        </div>
      </section>
      ...
      <section id="why" className="py-28 px-6 relative bg-black">
        ...
        <div className="max-w-6xl mx-auto">
          <FadeIn className="text-center mb-16">
            <p className="text-[#f5c518] text-xs uppercase tracking-widest mb-3 font-medium">{t('landing.why.label')}</p>
            <h2 className="text-4xl md:text-5xl font-bold">{t('landing.why.title')}</h2>
          </FadeIn>
              ...
              return (
                <FadeIn key={item.titleKey} delay={item.delay}>
                  <div className="group p-6 bg-[#0d0d0d] border border-white/[0.06] rounded-2xl hover:border-[#f5c518]/30 transition-all duration-300 h-full">
                    ...
                    <h3 className="font-semibold text-base mb-2 text-white">{t(item.titleKey)}</h3>
      ...
      <section className="py-28 px-6 bg-[#050505] relative">
        ...
        <div className="max-w-6xl mx-auto">
          <FadeIn className="text-center mb-16">
            <p className="text-[#f5c518] text-xs uppercase tracking-widest mb-3 font-medium">{t('landing.platform.label')}</p>
            <h2 className="text-4xl md:text-5xl font-bold">{t('landing.platform.title')}</h2>
            <p className="text-white/40 mt-4 max-w-lg mx-auto">
              {t('landing.platform.sub')}
            </p>
          </FadeIn>
          <FadeIn delay={0.15}>
          ...
          <div className="max-w-6xl mx-auto">
            <div className={`flex flex-col ${item.reverse ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-16`}>
              <FadeIn className="flex-1">
                ...
                <h2 className="text-3xl md:text-4xl font-bold leading-tight mb-5">{t(item.titleKey)}</h2>
                <p className="text-white/40 leading-relaxed mb-6 max-w-md">{t(item.descKey)}</p>
      ...
      <section className="py-28 px-6 bg-[#030303] relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          <FadeIn>
            <p className="text-[#f5c518] text-xs uppercase tracking-widest mb-3 font-medium">{t('landing.chain.label')}</p>
            <h2 className="text-4xl md:text-5xl font-bold mb-4">{t('landing.chain.title')}</h2>
            <p className="text-white/40 max-w-lg mx-auto mb-16">
              {t('landing.chain.sub')}
            </p>
          </FadeIn>
          <div className="grid md:grid-cols-3 gap-5 mb-12">
            {CHAIN_CARDS.map((item, i) => {
              const Icon = item.icon;
              return (
                <FadeIn key={item.titleKey} delay={i * 0.08}>
                  <div className="p-7 bg-[#0a0a0a] border border-white/[0.07] rounded-2xl text-left group hover:border-[#f5c518]/25 transition-colors h-full">
                    ...
                    <h3 className="text-base font-bold mb-3">{t(item.titleKey)}</h3>
                    <p className="text-white/40 text-sm leading-relaxed">{t(item.descKey)}</p>
                  </div>
                </FadeIn>
              );
      ...
      <section className="py-28 px-6 bg-black relative overflow-hidden">
        ...
        <FadeIn className="max-w-3xl mx-auto text-center relative z-10">
          <h2 className="text-4xl md:text-6xl font-bold mb-6 leading-tight">
            {t('landing.cta.title')}<br /><span className="text-[#f5c518]">{t('landing.cta.highlight')}</span>
          </h2>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-start justify-between gap-10 mb-10">
            <div>
              ...
              <p className="text-white/30 text-sm max-w-xs">{t('landing.footer.tagline')}</p>
            </div>
