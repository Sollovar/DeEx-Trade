...
export function MobileTradesView({ pairId }: Props) {
    ...
    const loadTrades = async () => {
      ...
      try {
        ...
        pairTrades.forEach((t) => {
          ...
          const decimals = typeof (t as any).decimals === "number" ? (t as any).decimals : fallbackDecimals;
          const size = normalizeTradeSize(
            t.amount_human ??
            (typeof t.amount === 'string' && !t.amount.includes('.') ? t.amount : undefined) ??
            t.amount,
            decimals
          );
          addTrade({
        ...
        userFills.forEach((fill) => {
          ...
          addTrade({
            id: fill.id,
            price: fill.price,
            size: normalizeTradeSize(fill.amount, fallbackDecimals),
            isBuy: fill.side === "buy",
  ...
  usePairWebsocket(resolvedPairId ?? null, {
    onTradeUpdate: (trade: TradeUpdatePayload) => {
      ...
      const decimals = trade.decimals ?? fallbackDecimals;
      const size = normalizeTradeSize(trade.amount_human ?? trade.amount, decimals);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ backgroundColor: "var(--m-bg)" }}>
      ...
      >
        ...
        <div className="flex items-center justify-end">Size ({baseSymbol})</div>
