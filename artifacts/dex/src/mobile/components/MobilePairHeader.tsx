...
export function MobilePairHeader({ market, currentSymbol, pair, flash, onOpenMarketPanel }: Props) {
  ...
  const geckoChange   = pair?.geckoPriceChange24h ?? pair?.priceChange24h ?? 0;
  ...
  // Flash overrides the direction colour momentarily when a real-time tick arrives
  const flashUpColor   = "#00ff7f";
  const flashDownColor = "#ff4d6a";
  const neutralUpColor   = "#00c853";
  const neutralDownColor = "#ff1744";
  const priceColor = flash === "up"   ? flashUpColor
                   : flash === "down" ? flashDownColor
                   : geckoChange >= 0  ? neutralUpColor
                   :                    neutralDownColor;
  ...
  const high24h        = pair?.priceHigh24h ?? pair?.high24h ?? (nativePrice > 0 ? nativePrice * 1.018 : 0);
  const low24h         = pair?.priceLow24h  ?? pair?.low24h  ?? (nativePrice > 0 ? nativePrice * 0.982 : 0);
          ...
          <div className="text-right flex flex-col leading-none gap-[3px]">
            <div
              ...
              style={{
                ...
                textShadow: flash ? `0 0 8px ${priceColor}99` : "none",
              }}
            >
              {geckoPrice > 0
                ? geckoPrice.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 6 })
                : "—"}
              {geckoPrice > 0 && (
                <span className="text-[11px] font-semibold ml-1.5" style={{ color: priceColor }}>
