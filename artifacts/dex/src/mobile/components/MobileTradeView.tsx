...
export function MobileTradeView({ market, currentSymbol, pair, onOpenMarketPanel }: Props) {
  ...
  const geckoChange = pair?.geckoPriceChange24h ?? pair?.priceChange24h ?? (market.change24h * 100);
  const geckoUp = geckoChange >= 0;
  const geckoColor = geckoUp ? "#00c8a0" : "#ff4d6a";
  ...
  const realPrice = geckoPriceUSD ?? geckoPrice;
          ...
          <div className="px-3 pt-3 pb-3 flex flex-col gap-2.5">
            ...
            {orderType !== "Ladder" && (
              <div className="flex flex-col gap-2.5">
                <Check checked={postOnly} label="Post Only" onChange={() => setPostOnly(!postOnly)} />
                <Check checked={reduceOnly} label="Reduce Only" onChange={() => setReduceOnly(!reduceOnly)} />
