...
const TICK_OPTIONS = [0, 0.001, 0.01, 0.1, 1, 10, 50];
...
/* ── Depth chart ── */
function DepthChart({ market, bookUnit }: { market: LiveMarketState; bookUnit: "base" | "quote" }) {
  const N     = 20;
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ backgroundColor: "var(--m-bg)" }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setBookUnit(u => u === "base" ? "quote" : "base")}
            className="flex items-center gap-1 text-[12px] font-medium transition-colors"
            style={{ color: bookUnit === "quote" ? "#f5c518" : "var(--m-fg-2)" }}
          >
      ...
      {/* Depth chart */}
      {tab === "depth" && <DepthChart market={market} bookUnit={bookUnit} />}
        ...
        <>
          ...
          >
            <div className="text-[11px] font-medium px-3 flex items-center" style={{ color: "var(--m-fg-4)" }}>
              Total ({bookUnit === "base" ? base : quote})
            </div>
