...
/* ─────────────────────────── spark helpers ─────────────────────────── */
function makeSpark(price: number, changeDir: number): number[] {
  const pts: number[] = [];
  let v = price * (1 - changeDir * 0.03);
  for (let i = 0; i < 14; i++) {
    v += price * (Math.random() * 0.012 - 0.005 + changeDir * 0.003);
    pts.push(v);
  }
  return pts;
}

/* memoize sparklines per pair so they don't regenerate every render */
const sparkCache = new Map<string, number[]>();
function getSpark(pair: NormalizedPair): number[] {
  if (!sparkCache.has(pair.id)) {
    sparkCache.set(pair.id, makeSpark(pair.price, pair.change24h > 0 ? 1 : pair.change24h < 0 ? -1 : 0));
  }
  return sparkCache.get(pair.id)!;
}
...
function Spark({ data, color, w = 56, h = 24 }: { data: number[]; color: string; w?: number; h?: number }) {
  ...
  const area = `M${coords[0].x},${coords[0].y} ` +
    ...
    ` L${w},${h} L0,${h} Z`;
  const gid = `sp-${color.replace(/[^a-z0-9]/gi, "")}-${data.length}`;
  return (
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: pair.color + "30", border: `1.5px solid ${pair.color}40`, color: pair.color,
      }}
    >
      {pair.letter}
    </div>
...
/* ─────────────────────────── movers card ──────────────────── */
function MoverCard({ p, onSelect }: { p: NormalizedPair; onSelect: () => void }) {
...
export function MobileMarketsPage({ market, currentSymbol, onSelectPair, onOpenMarketPanel }: Props) {
  ...
  const [alertPair, setAlertPair] = useState<NormalizedPair | null>(null);
  const pressTimer      = useRef<ReturnType<typeof setTimeout> | null>(null);
  const pressMove       = useRef(false);
  const headerScrollRef = useRef<HTMLDivElement>(null);
  const bodyScrollRef   = useRef<HTMLDivElement>(null);

  const { pairs, loading } = usePairs({ limit: 100 });
  ...
  const startPress = useCallback((pair: NormalizedPair) => {
  ...
  /* ── Live price simulation seeded from backend prices ── */
  const initialPrices = useMemo(
    () => Object.fromEntries(pairs.map(p => [p.symbol, p.price])),
    [pairs],
  );
  const livePrices = useLivePrices(initialPrices);

  const topGainers = useMemo(() => [...pairs].sort((a, b) => b.change24h - a.change24h).slice(0, 4), [pairs]);
  const topLosers  = useMemo(() => [...pairs].filter(p => p.change24h < 0).sort((a, b) => a.change24h - b.change24h).slice(0, 4), [pairs]);

  const filtered = useMemo(() => {
    let list = [...pairs];
    if (tab === "Favorites") list = list.filter(p => favorites.has(p.symbol));
    if (tab === "Gainers")   list = list.filter(p => p.change24h > 0);
    if (tab === "Losers")    list = list.filter(p => p.change24h < 0);
    if (search.trim()) {
      const q = search.trim().toUpperCase();
      list = list.filter(p => p.symbol.includes(q) || p.base.includes(q) || p.quote.includes(q));
    }
    if (sortBy === "volume")    list.sort((a, b) => b.volume    - a.volume);
    if (sortBy === "change")    list.sort((a, b) => b.change24h - a.change24h);
    if (sortBy === "price")     list.sort((a, b) => b.price     - a.price);
    if (sortBy === "liquidity") list.sort((a, b) => b.liquidity - a.liquidity);
    return list;
  }, [tab, search, favorites, sortBy, pairs]);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden" style={{ paddingBottom:60 }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* Loading state */}
        {loading && (
          <div className="flex items-center justify-center py-16 gap-2" style={{ color:"var(--m-fg-4)" }}>
            <Loader2 className="w-5 h-5 animate-spin" />
            <span className="text-[13px]">Loading pairs…</span>
          </div>
        )}
            ...
            {tab === "All" && !search && pairs.length > 0 && (
              <div className="px-3 pt-3">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1 mb-3">
                  {topGainers.map(p => <MoverCard key={p.id} p={p} onSelect={() => onSelectPair?.(p.symbolSlash)} />)}
                </div>
                <div className="flex items-center gap-1.5 mb-2">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1 mb-2">
                  {topLosers.map(p => <MoverCard key={p.id} p={p} onSelect={() => onSelectPair?.(p.symbolSlash)} />)}
                </div>
                    ...
                    {filtered.map(pair => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => onSelectPair?.(pair.symbolSlash)}
                        ...
                        >
                          ...
                          <div className="flex items-center flex-1">
                            ...
                            <div className="flex items-center justify-end shrink-0 pr-3" style={{ width:68 }}>
                              <Spark data={getSpark(pair)} color={sparkColor} w={56} h={24} />
                            </div>
                          </div>
                        </div>
                      );
