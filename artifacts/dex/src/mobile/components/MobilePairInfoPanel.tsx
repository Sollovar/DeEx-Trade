  ...
  return (
    <div className="overflow-y-auto h-full" style={{ paddingBottom: 80 }}>
      <div className="px-4">
        ...
        {/* ── price ── */}
        <SHead label="Price" />
        <SRow label="Price (native)" value={fmtPrice(p.price)} />
        <SRow label="Price USD"      value={fmtUsd(p.priceUSD)} />
        <SRow
          label="24h Change"
          value={`${up ? "+" : ""}${(p.priceChange24h ?? 0).toFixed(2)}%`}
          accent={up ? "#00c853" : "#ff4d6a"}
        />
        <SRow label="24h High" value={fmtPrice(p.priceHigh24h ?? p.high24h ?? 0)} />
        <SRow label="24h Low"  value={fmtPrice(p.priceLow24h  ?? p.low24h  ?? 0)} />
