...
export function TradingPairHeader({ market }: Props) {
  ...
  const activePair = currentPairId
    ? pairs.find((p) => p.id === currentPairId) ?? pairs[0]
    : pairs[0] ?? null;

  const baseSymbol  = activePair?.baseToken?.symbol  ?? currentSymbol?.split("/")[0] ?? "—";
  const quoteSymbol = activePair?.quoteToken?.symbol ?? currentSymbol?.split("/")[1] ?? "—";
    ...
    <>
      ...
      {panelOpen && (
        <PairSelectorPanel
          ...
          currentSymbol={currentSymbol ?? (activePair ? `${activePair.baseToken?.symbol}/${activePair.quoteToken?.symbol}` : "")}
        />
      )}
    </>
  );
