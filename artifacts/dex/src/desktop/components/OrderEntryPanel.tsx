...
const INPUT_ROW = "flex items-center bg-[#111] border border-[#222] focus-within:border-[#3a3a3a] px-3 h-[40px] gap-2 transition-colors rounded-sm";

export function OrderEntryPanel({ market, symbol = "BTCUSDT" }: Props) {
  const baseToken = symbol.replace(/USDT.*/, "").replace("/", "") || "BTC";
  ...
  const displayPrice = market.price.toLocaleString("en-US", { minimumFractionDigits: 1, maximumFractionDigits: 1 });
    ...
    <div className="flex flex-col bg-[#0a0a0a] h-full">
      ...
      <div className="flex items-center h-[38px] px-3 border-b border-[#1a1a1a] bg-[#0e0e0e] shrink-0 gap-5">
        {(["Limit", "Market", "Ladder"] as const).map((t) => (
          <button
            ...
            style={{
              color: tab === t ? (t === "Ladder" ? "#a78bfa" : "#fff") : "#555",
              borderBottom: tab === t ? `2px solid ${t === "Ladder" ? "#a78bfa" : "#f5c518"}` : "2px solid transparent",
            }}
          >
            {t}
          </button>
      ...
      <div className="p-3 flex flex-col gap-3 overflow-y-auto">
        ...
        <div className="flex items-center justify-between text-[12px]">
          ...
          <span className="text-[#888] font-mono">0.00 USDT</span>
        </div>
        ...
        {tab === "Limit" && (
          <div className={INPUT_ROW}>
            ...
            />
            <span className="text-[11px] text-[#555] shrink-0">USDT</span>
          </div>
        )}
        ...
        <div className="flex gap-1.5">
          <button
            ...
            className="flex-1 py-2 text-[13px] font-bold transition-all rounded-sm"
            style={{
        ...
        <div className="flex flex-col gap-1">
          <div className={INPUT_ROW}>
            ...
            >
              {sizeUnit === "base" ? baseToken : "USDT"} <ChevronDown className="w-3 h-3" />
            </button>
          </div>
          {tokenEquiv && (
            ...
            {ladderValid ? (
              <>
                {[
                  ...
                  ["Price Interval",  `${ladderInterval!.toFixed(3)} USDT`,         "#888"],
        ...
        <button
          className="w-full font-bold py-3 text-[14px] transition-colors mt-1 rounded-sm"
          style={{
            backgroundColor: tab === "Ladder" ? "#a78bfa" : "#f5c518",
            color: tab === "Ladder" ? "#fff" : "#000",
          }}
        >
          {tab === "Ladder" ? "Place Ladder Order" : "Connect Wallet"}
        </button>
      </div>
    </div>
  );
